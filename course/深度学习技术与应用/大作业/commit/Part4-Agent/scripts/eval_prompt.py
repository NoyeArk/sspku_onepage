"""
BrowseComp-ZH 评测 Prompt（适配 DeepDiver V2）
- 去掉了置信度（Confidence）部分，因为 DeepDiver 不输出置信度
- Judge prompt 也相应去掉了 confidence 字段的提取
"""

JUDGE_PROMPT_CN = """根据以下精确且明确的[correct_answer]，判断以下对[question]的[response]是否正确。
