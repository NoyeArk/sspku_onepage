...
def sort_batch_by_length(tensor, sequence_lengths):
    """
    Sort a tensor by its sequence lengths (descending).
    Replacement for allennlp.nn.util.sort_batch_by_length.
    """
...
def shiftLeft(t, pad):
  ...
  padding =  torch.zeros(t.size(0), 1).fill_(pad).long().to(device_utils.get_device())
