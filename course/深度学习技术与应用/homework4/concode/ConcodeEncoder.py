...
class ConcodeEncoder(nn.Module):
  ...
  def forward(self, batch):
    ...
    src = Variable(batch['src'].to(device_utils.get_device()), requires_grad=False)
    ...
    if self.opt.var_names:
      ...
      varCamel = Variable(batch['varNames'].transpose(1, 2).contiguous().to(device_utils.get_device()), requires_grad=False)
      if self.opt.nocamel:
      ...
      varTypes = Variable(batch['varTypes'].to(device_utils.get_device()), requires_grad=False)
    ...
    if self.opt.method_names:
      methodCamel = Variable(batch['methodNames'].transpose(1, 2).contiguous().to(device_utils.get_device()), requires_grad=False)
      if self.opt.nocamel:
      ...
      methodReturns = Variable(batch['methodReturns'].to(device_utils.get_device()), requires_grad=False)
    ...
    src_attention_mask = Variable(batch['src'].ne(self.vocabs['names_combined'].stoi['<blank>']).to(device_utils.get_device()), requires_grad=False)
    var_attention_mask = Variable(batch['varTypes'].ne(self.vocabs['types'].stoi['<blank>']).unsqueeze(2).expand(batch_size, max_var_len, 2).contiguous().view(batch_size, -1).to(device_utils.get_device()), requires_grad=False)
    method_attention_mask = Variable(batch['methodReturns'].ne(self.vocabs['types'].stoi['<blank>']).unsqueeze(2).expand(batch_size, max_method_len, 2).contiguous().view(batch_size, -1).to(device_utils.get_device()), requires_grad=False)
