...
class RegularGenerator(nn.Module):
  ...
  def computeLoss(self, scores, batch):
    target = Variable(shiftLeft(batch['code'].to(device_utils.get_device()), self.vocabs['code'].stoi['<blank>']).view(-1), requires_grad=False)
...
class ProdGenerator(nn.Module):
  def __init__(self, rnn_size, vocabs, opt):
    ...
    self.opt = opt
    self.mask = Variable(vocabs['mask'].float().to(device_utils.get_device()), requires_grad=False)
  ...
  def forward(self, hidden, attn, src_map, batch):
    ...
    non_terminals = batch['nt'].contiguous().to(device_utils.get_device()).view(-1)
    ...
    masked_copy = Variable(non_terminals.to(device_utils.get_device()).view(-1, 1).eq(self.vocabs['nt'].stoi['IdentifierNT']).float()) * copy
    ...
    copy_prob = torch.bmm(mul_attn.view(batch_size, -1, slen), Variable(src_map.to(device_utils.get_device()), requires_grad=False))
  ...
  def computeLoss(self, scores, batch):
    ...
    target = Variable(batch['next_rules'].contiguous().to(device_utils.get_device()).view(-1), requires_grad=False)
    if self.opt.decoder_type == "prod":
      align = Variable(batch['next_rules_in_src_nums'].contiguous().to(device_utils.get_device()).view(-1), requires_grad=False)
      align_unk = batch['seq2seq_vocab'][0].stoi['<unk>']
    elif self.opt.decoder_type in ["concode"]:
      align = Variable(batch['concode_next_rules_in_src_nums'].contiguous().to(device_utils.get_device()).view(-1), requires_grad=False)
...
class CopyGenerator(nn.Module):
    ...
    def computeLoss(self, scores, batch):
        ...
        self.target = Variable(shiftLeft(batch['code'].to(device_utils.get_device()), self.tgt_padding_idx).view(-1), requires_grad=False)

        align = Variable(shiftLeft(batch['code_in_src_nums'].to(device_utils.get_device()),  self.vocabs['seq2seq'].stoi['<blank>']).view(-1), requires_grad=False)
