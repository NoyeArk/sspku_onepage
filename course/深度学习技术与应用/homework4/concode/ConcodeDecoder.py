...
class ConcodeDecoder(nn.Module):
  ...
  def forward(self, batch, all_context, context_masks, decState):
    ...
    # embed everything
    nt_embeddings = self.nt_embedding(Variable(batch['nt'].to(device_utils.get_device()), requires_grad=False))
    rule_embeddings = self.rule_embedding(Variable(batch['prev_rules'].to(device_utils.get_device()), requires_grad=False))
    parent_rule_embeddings = self.rule_embedding(Variable(batch['parent_rules'].to(device_utils.get_device()), requires_grad=False))
  ...
  def predict(self, enc_hidden, context, context_lengths, batch, beam_size, max_code_length, generator, replace_unk, vis_params):
    ...
    decState = DecoderState(
      ...
      Variable(torch.zeros(1, 1, self.opt.decoder_rnn_size).to(device_utils.get_device()), requires_grad=False) # parent state
    )
